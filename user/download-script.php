...
if(isset($_GET['file_name'])) { // Check if a file name has been provided
    $script_name = mysqli_real_escape_string($con, $_GET['file_name']);
    $sql = "SELECT * FROM `script` WHERE `script_name`='$script_name'";
    $result = mysqli_query($con, $sql);
    $file = mysqli_fetch_assoc($result);
    $file_path = "../script/" . $_GET['file_name']; // Set the path to the file using the file name
    ...
    if (file_exists($file_path)) {
      ...
      readfile($file_path);
      $newCount = $file['script_downloads'] + 1;
      $updateQuery = "UPDATE `script` SET `script_downloads`=$newCount WHERE `script_name`='$script_name'";
...
if(isset($_GET['files_name'])) {
    ...
    $result = $con->query("DELETE from script WHERE script_path = '$filePath' ");
    if(!$result) {
        echo $con->error;
    } else {
...
<html
  ...
  <head>
    ...
    <title>Download Script | <?php echo ''.$server_data['server_name'].'';?></title>
    ...
    <div class="layout-wrapper layout-content-navbar">
      ...
      <?php include('header.php'); ?>
        ...
        <?php } ?>
              ...
              <!-- Select -->
              <div class="card">
                <h5 style="margin-bottom: -30px; margin-top: -10px;" class="card-header">Download Script</h5>
                <hr>
                ...
                <div class="table-responsive">
                  <table id="example" class="table table-striped table-bordered display nowrap" style="width:100%">
                    <thead>
						...
						<tbody>
							<?php
									$query_scripts = mysqli_query($con,"SELECT * FROM script ORDER BY id ASC");
							     	while ($row = mysqli_fetch_assoc($query_scripts)) {
									?> <tr>
                  <td class="table-plus"><?php echo $row['script_name_show']; ?></td>
                  <td><?php echo $row['script_version']; ?></td>
                  	<th><?php 
										if($row['script_status'] == "online"){
											echo "<span class='badge border border-success text-success mt-1'><i class='fas fa-circle-check '></i> Online</span>"; 
										}else if($row['script_status'] == "maintain"){
										...
										?></td>
                  <td><?php echo $row['script_size'] / (1024 * 1024), 2; ?> Mb</td>
                  <td><?php echo $row['script_downloads']; ?></td>
                  <td><?php echo $row['created_at']; ?></td>
                                      <td><a href="?file_name=<?php echo $row['script_name']; ?>" download="?file_name=<?php echo $row['script_name']; ?>"><button type="button" class="btn btn-primary">Download</button></a></td>
